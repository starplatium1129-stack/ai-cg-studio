...
.material-switch { position: relative; isolation: isolate; display: flex; gap: var(--s-1); padding: 0 var(--s-3) var(--s-3); border-bottom: 1px solid var(--border-soft); }
.material-switch button { position: relative; z-index: var(--z-raised); flex: 1; min-width: 0; min-height: 42px; display: flex; align-items: center; justify-content: center; gap: var(--s-1); border: 1px solid transparent; border-radius: var(--r-md); background: transparent; color: var(--text-secondary); font: 500 var(--fs-label-sm) var(--font-sans); cursor: pointer; }
