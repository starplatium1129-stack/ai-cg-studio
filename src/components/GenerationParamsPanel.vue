  ...
  <details class="panel generation-settings">
    ...
    <div class="controls-grid">
      <div class="ctrl"><label>CFG</label>
        <select v-model.number="params.cfg" @change="touch('cfg')">
          <option v-for="v in [3,4,4.5,5,5.5,6,7,8]" :key="v" :value="v">{{ v }}</option>
        </select>
      </div>
      <div class="ctrl"><label>Steps</label>
        <select v-model.number="params.steps" @change="touch('steps')">
      ...
      <div class="ctrl"><label>Sampler</label>
        <select v-model="params.sampler" @change="touch('sampler')">
          <option v-for="sampler in samplerOptions" :key="sampler">{{ sampler }}</option>
        </select>
      </div>
      <div class="ctrl"><label>Scheduler</label>
        <select v-model="params.scheduler" @change="touch('scheduler')">
      ...
      <div v-if="params.negative" class="ctrl ctrl-full negative-editor">
        <label>负面提示词</label>
        <textarea v-model="params.negativeCustom" placeholder="留空使用默认负面；可追加如：extra fingers, bad anatomy"></textarea>
      </div>
    </div>
  </details>
</template>
