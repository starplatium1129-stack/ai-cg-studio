...
onMounted(() => {
  ...
  before = router.beforeEach(() => { finish(); if (!media?.matches) timer = window.setTimeout(() => { loading.value = true }, 180) })
  after = router.afterEach(finish)
  error = router.onError(finish)
})
onUnmounted(() => { finish(); before?.(); after?.(); error?.(); media?.removeEventListener('change', finish); document.removeEventListener('pointerover', prefetch); document.removeEventListener('focusin', prefetch); document.removeEventListener('pointerdown', prefetch); document.removeEventListener('click', click) })
</script>
<style scoped>
