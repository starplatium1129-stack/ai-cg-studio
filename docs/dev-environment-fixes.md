...
### 7.3 v3：行为模型逐项对齐参考实现（2026-08-16 三次迭代）

用户对比指出"不是一回事"后，把参考实现的物理/渲染行为逐项对齐（颜色保留
角色配色，用户决策"有颜色更好看"）：

| 行为 | 参考值（js/index.js） | 实现 |
|---|---|---|
| 待机漂移 | 无（完全静止） | 剪影模式漂移归零（此前 0.3 在糊画面） |
| 斥力半径 | 105px 固定 | portrait 恒 105px |
| 斥力衰减 | ratio² × 1.8 | 同 |
| 回位弹簧 | 恒 0.01（慢回流=流动感） | 同（抽象形状维持原分档弹簧） |
| 摩擦 | 0.15/帧 | 同 |
| 点击脉冲 | 无 | 剪影模式关闭 |
| 点径/点距 | 3px/5px = 0.6 | 0.62（略放大适配小场域） |

对齐后成像相符度 10/10（"边缘锐利无拖影、缝隙均匀规整、点阵感极强"）。

### 7.4 v4：全站粒子统一为参考行为（2026-08-16）

用户决策"其他页面的粒子效果也抄过来"：参考行为不再只用于剪影模式，
`SemanticParticleField` 的抽象形状（首页椭圆轨道/场景山形/画风星芒/各页
frame/moon/book/spark 等）同样切换——静止成像（删除待机漂移）、统一点径
（0.62×平均点距，按点数与场域面积估算）、105px 斥力 + 0.01 慢回流 + 0.15
摩擦、删除点击脉冲与指针高光（参考实现均无）。三档 tone 仅保留为配色分层。
页面识别色不变（每页 ROUTE_META 的形状 + 主题色），只统一"行为与质感"。
视觉复核：首页/灵感场景/画风三页点阵锐利静止、点径一致、轮廓完整。

### 7.5 v5：整图完整复刻（2026-08-16）

用户问「能否将图片完整复刻」——对齐参考实现的本质（任意图片整图点阵，不抠
背景）：

- **生成端 v3**：去掉 rembg（管线只剩 numpy+Pillow，秒级跑完 35 角色）；
  整图 LANCZOS 降到 200px 宽 → k-means **32 主色**（base36 索引字符画，
  单角色 ~86KB/原图）→ 无抠像、背景完整保留。
- **前端**：base36 解码；点数 hero 8000 / ambient 6000 / 窄屏 2400；
  点径 0.55×点距（更密的点承载 32 色层次，同时保留点阵缝隙质感）。
- **效果**：原图 vs 页面点阵 A/B 视觉对比 **8.8/10**（人物+背景构图完整、
  32 色无明显断层，一眼可辨为原图场景；差距仅在发卡/雕花/丁达尔光等
  细节，属点阵分辨率固有极限，参考实现同样如此）；6000 粒子 165fps。
- 人物抠像版（v1/v2 的 rembg 剪影管线）见 git 历史，如需回到"只保留人物"
  的观感可从历史恢复。

### 7.6 抽象形状恢复三档点径（2026-08-16 用户决策）

v4 把"统一点径"推广到抽象形状后用户反馈"好粗好难看"——稀疏的轮廓形状
（轨道/山形/星芒）配大而统一的点径显得粗笨。最终格局：
- **图片点阵**（角色场景/角色档案 hero）：统一点径 0.55×点距（密集成像需要）；
- **抽象形状**（灵感场景/画风/各页 ROUTE_META 形状）：恢复经典三档点径
  0.78/1.05/1.55 + 三档 tone 配色（层次感与强调色亮点）；
- **动效全站统一**不变：静止成像、105px 斥力、0.01 慢回流、无脉冲/高光。

### 7.7 v6：融合与提色（2026-08-16）

用户反馈"粒子和其他地方显得突兀，不能很好融合"（附截图，浅色主题）+ 希望颜色
还原度继续提升：

- **提色**：k-means 32 → **36 色**（base36 上限），量化断层再压一档。
- **融合四件套**：
  1. 深色主题 canvas `globalCompositeOperation='screen'`——暗部自然隐入页面
     底色、亮部呈发光感，消除"贴上去的马赛克"（浅色主题正常混合，透明度
     .88→.76 收敛暗部硬度）；
  2. 生成端**边缘羽化**：四边 14% 区域 smoothstep 概率渐剔除，边缘点渐稀
     消散，矩形边界不再生硬；
  3. 生成端**近黑格剔除**（luminance<0.05）：去泥感省点数；
  4. 图片点阵激活时**隐藏画布中央装饰菱形**（透在图片上读成"幽灵图元"）。
- 视觉复核：菱形已无残留、边缘消散自然、与浅粉紫主题过渡协调；水蓝发色/
  黑白女仆装/拱窗暖光还原到位（发卡/瞳孔高光等微细节仍受点径限制——点阵
  分辨率固有极限）。

### 7.8 desynchronized canvas 黑屏回归（教训）
