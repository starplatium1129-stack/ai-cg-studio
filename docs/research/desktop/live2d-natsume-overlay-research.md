...
> 2026-09-11 状态核对：本文保留 8 月 15 日的研究过程，下方未勾选项不再作为当前待办。后续已实现参数隐藏态白名单、动作所有权交接及平滑回落，见[运行时维护记录](../../guides/desktop/live2d-native-runtime.md)和[重构完成验收](../../archive/completed/live2d-composable-refactor-plan.md)。本文早期“统一复位为 0”的设想已被默认值实测修正；本次仅核对文档与实现，未重新进行原生视觉验收。
