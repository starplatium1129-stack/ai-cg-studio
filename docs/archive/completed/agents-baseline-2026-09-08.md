...
> 历史快照，含已失效的版本、主题与规模信息；不再作为执行规范。当前入口：[AGENTS.md](../../../AGENTS.md)。
...
4. **内容分级 Fail-Closed 契约（2026-08-31 修订：本机默认全 NSFW）**：成人（R18）内容**本机默认全开**——所有新加热门角色默认配 NSFW 场景 + 显式形态，数据层 `adultEligibility: "adult"` 为默认值无需逐角色判定/override；但**远程/隧道访问**仍 fail-closed：`adultEnabled = isLocalStudioHost()`（本机放行、远程/隧道拒绝 R18 并带模糊遮罩），未知或未授权状态必须严格拒绝，不得回退安全断言。即"本机无门槛、远程严门控"。
5. **精准提交与工作区保护**：
