...
> ⚠️ **2026-08-28 过期标注**：文中多处把「light 主题」当作审计基线，与当日**浅色主题已下线（锁定深色，`src/composables/useTheme.ts` `LOCKED_THEME='dark'`，toggle 已移除）**直接冲突；文中双主题对比度结论已不适用于当前运行态。以 `AGENTS.md` 第一节与 `DESIGN.md` 为准，本文件仅保留历史演进记录。
