...
> 状态：P0（长按说话最小闭环）、P1（会话状态机/唤醒词）、P2（LLM 情绪标签协议）已完成并交付（见 AGENTS.md「桌宠语音与演出增强」）；P3（演出数据驱动）/P4（自定义角色资产包）暂缓，经批准后按 P3 → P4 顺序逐阶段启动，每阶段独立验收、不串阶段。CompanionView 的自动监听/热键集成待其未提交改动落地后进行。
