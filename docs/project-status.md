...
- 绘图：场景模式是一键流程，只需选择预设场景与底模；镜头、光照、构图、Prompt 和模型参数自动确定。WAI v17 普通兼容请求仍为 Comfy-first；自动 hires 则优先 WebUI Anime6B，仅 Comfy 可用时退到 nearest-exact Latent。Anima Base/Aesthetic 使用 30 steps / CFG 4.5 / `euler_ancestral` / `simple`（放大 = Remacri 纯像素直出）的模型原生标签流。Krea 2 Turbo 使用 3~5 句纯英文自然语言且无负面。
...
- **36 角色体系与一站式角色接入自动化引擎（Character Onboarding Pipeline）**：
...
- **桌面端增量极速部署流水线（`deploy-desktop-quick.ps1`）**：
  - 一键完成 build + data-first 资源同步 + WebView2 缓存穿透清理 + 僵尸 chunk 裁剪 + 静默重启，大幅提升桌面端迭代效率。
