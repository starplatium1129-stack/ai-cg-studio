...
AI-CG-Studio 是本地个人使用的 Galgame 风格 AI CG 创作台，包含角色聊天、Live2D、TTS、SD/Anima 出图、LoRA 训练、场景库、作品册、控制面板和桌面 Companion。
...
- 绘图：SD/WAI 为生产主路径；Anima 为受白名单保护的实验路径。WAI 基础 txt2img 在 WebUI 在线时优先，WebUI 离线且 ComfyUI 在线时可使用固定核心节点 fallback。
...
- ComfyUI 原始接口已从浏览器封闭为应用级 Anima/WAI job API；固定 workflow、模型/LoRA/参数白名单、路径 containment、取消、TTL 和结果 MIME 校验均由服务端控制。
...
- 后续若扩展能力，必须先有固定服务端契约、真实 GPU 证据和逐图人工审核。当前不启动 Krea 2 或视频生成。
...
- `npm run validate`：205/205 unit；contract 数量不在本文固定，按当前质量清单执行。
