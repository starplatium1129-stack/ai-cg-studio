...
- **SageAttention2 内核加速已上线（2026-08-25 实测转正）**：与 TeaCache 正交叠加（TeaCache 跳步、Sage 加速剩余步算子）。`sageattention 2.2.0` wheel（cu130torch2.13.0/cp311，与本机 venv 逐版本匹配）+ `triton-windows 3.6.0.post26` 装入 venv，启动参数加 `--use-sage-attention`（`start-comfyui.ps1` 与 `managed-comfyui.ps1` 两条路径必须同步）。同基准三连测（832×1216/30 步/res_multistep/TeaCache 0.08）：**TeaCache 链 12.73s → 11.35s（-1.38s，-11%）**；无 TeaCache 链 23.00s → 20.21s（-12%）。质量：本机渲染位确定性已验证（同 seed 同配置重跑 PSNR=inf），sage on/off 同 seed 对比 PSNR 22.76dB、diff p50=4/255（半数像素差≤4，热点集中在 1% 发丝边缘区），属注意力量化典型扰动档而非结构漂移（结构漂移对照实测 11.86dB）；契约回归 8/8+24/24 全绿。`ComfyUI attention_sage()` 自带 per-call 回退保护，kernel 异常时自动降级 PyTorch attention 不炸链路。
