...
- **SageAttention2 接入边界已核实（2026-08-25）**：`sageattention 2.2.0` + `triton-windows 3.6.0.post26` 已装入 venv，两个启动路径保留 `--use-sage-attention`，供走 ComfyUI 全局 attention dispatcher 的 Krea2/视频模型使用；但 Anima 的 `comfy/ldm/anima/model.py` 第 80 行直接调用 `torch.nn.functional.scaled_dot_product_attention`，绕过该 dispatcher，因此**不能把 SageAttention 计入 Anima 主干提速**。此前 12.73s→11.35s 的归因撤销，需针对 Anima 改模型 attention 接线路径后重新实测。临时 `fp16_accumulation` 测试虽将 Anima 切到 fp16，TeaCache 链 11.35s→10.33s，但当前同 seed 数值门禁未通过（正确配对最低 PSNR 约 11.92dB，且视觉后端当时不可用），已回滚，不进入生产；不能据此宣称质量等价。契约回归 8/8+24/24 全绿。
