...
- ✅ 真实 GPU 批量生成和人工审核 —— 已通过 2026-08-14/15 场景与热门角色 showcase 轮次完成（v16-v22：298 场景 + 186 热门样张，逐张 gemini 八维初筛 + 用户目检定稿 + 事实性定级复核，详见 `showcase-generation-craft.md` §9）
- ✅ 训练侧统一重训（含 safe prompt 泄漏测试硬门槛）—— 2026-08-15 用户确认：训练已收官（v21 LoRA 已实机用于 showcase 轮次），短时间内不再重训 LoRA
- ⏳ 去掉生成侧 `nene_r18 / natsume_r18` 质量 hack —— 已不被训练阻塞（`promptPolicy.ts` R18 控制词注入仍在），随时可做，需按路线图先 A/B 验证还原度不降再移除
- ✅ ">=90 分比例 >70%"最终验收 —— 以 showcase 轮次人工审核通过率作为验收证据（用户确认口径）
