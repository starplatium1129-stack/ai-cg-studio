...
> 更新于 2026-09-11。只列待办、待复验与待决策事项；已完成记录见 [归档](archive/README.md)。优先级表示建议次序，不代表已启动或承诺交付日期。
...
| V1／P2 | 代码已落地、浏览器复验未做：`src/router/documentPolicy.ts` 改为按已加载文档判定，不再依赖 `sessionStorage` 标记与 `evalAllowed()` 探测，`/chat/`、`/companion?src=x` 等变体一并覆盖（单测 6 条，含存储被禁用场景）；旧实现正是[复核清单](archive/audits/project-review-checklist-2026-09-10.md)第 117 行推断的缺陷来源 | 使用全新浏览器上下文检查直达、返回与存储异常 | 进出 Live2D 的文档策略正确，不扩大全站 unsafe-eval |
