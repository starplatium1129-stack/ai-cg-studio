...
依据：[项目评价与优化／修复清单](archive/audits/project-review-checklist-2026-09-10.md)；[机器复现证据](evidence/project-review-2026-09-10.json)。以下均未修复／未完成，不把隔离复现断言通过视为修复通过。初始代码基线为 `1106904`，交付前已核对数据源合并 `576e0e1`，请求层代码未变。

| ID／优先级 | 当前状态与事项 | 下一步 | 验收条件 |
| --- | --- | --- | --- |
| R1／P1 | 已复现：写成功后旧 GET 回填缓存；聊天配置存在对应读写路径 | 增加资源代际，阻止旧读回填与写后搭乘旧 inflight | POST／DELETE 与迟到读取交错时保持新配置，保留取消替换回归 |
| R2／P2 | 已复现：缓存／共享响应跳过本调用 validate | 分离共享传输与消费者校验 | fresh／cached／shared 均校验，互不连带 |
| R3／P2 | 已复现：可变响应对象与缓存／消费者共享引用 | 明确不可变契约或隔离返回对象，并评估开销 | 顶层、嵌套和数组修改不会污染其他结果 |
| O1／P2 | 契约优化：没有清晰的内存缓存绕过策略；不是 HTTP no-store 与 TTL 并存本身有错 | 明确 bypass／refresh 与 inflight 语义 | 强制刷新可测，聊天配置的既有 30 秒内存去抖不被意外移除 |
| V1／P2 | 待浏览器复验：Live2D 直达和 sessionStorage 不可用时的 CSP 切换 | 使用全新浏览器上下文检查直达、返回与存储异常 | 进出 Live2D 的文档策略正确，不扩大全站 unsafe-eval |
| O2／P2 | 测试优化：既有请求层测试尚需补本次边界场景 | 在 client.spec.ts 补回归，为 API 层建立可执行覆盖基线 | 新断言旧代码失败、修复后通过，完整质量门无回归 |
| O3／P2 | 性能优化：旧验收记录的绘图路由余量较薄，需当前构建复测 | 优先按需加载低频动作，记录路由与静态闭包余量 | 不放宽原硬预算；冷／热进入和首次操作有实测 |
| O4／P2 | CI 优化：Quality 的重复提交检查缺少并发收敛 | 普通验证按 workflow＋分支／PR 控制并发，评估文档检查分支 | 过期任务可取消，必需状态正常完成，未知代码路径仍全量验证 |
...
报告中 C1 对应上表原有故事／提示词任务：当前优先静态与编译错误排查，人工看图继续后置；后续改动仍遵守真实渲染与 pinned 保护，未看图不记为视觉通过。C2 对应角色／素材及批次对账：`576e0e1` 已合并的 16 蓝图／4 服装批次不再记为草稿，但数量与审核状态须重建后逐 ID 核对，不能仅凭合并说明认定全部验收。聚合数据是被忽略的生成物，不要求提交它们。

V2 沿用原 P0 的发布前默认素材验收，V3 沿用原 P1 的原生取消／语音与桌面验收，不另开重复任务，也不为本次文档交付启动 GPU 或安装程序。建议先收口 R1 与同模块回归，再按用户安排推进 C1；素材／真机验收在既定环境进行。
