...
V2 沿用原 P0 的发布前默认素材验收，V3 沿用原 P1 的原生取消／语音与桌面验收，不另开重复任务，也不为本次文档交付启动 GPU 或安装程序。R1 与同模块回归已在办公机收口，定稿保护的处置也已写入记录；建议下一步优先处理数据内容契约与 pinned 场景的批量任务豁免，素材／真机验收在既定环境进行。
