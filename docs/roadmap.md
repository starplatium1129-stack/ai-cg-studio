...
> 更新于 2026-09-10。只列待办、待复验与待决策事项；已完成记录见 [归档](archive/README.md)。优先级表示建议次序，不代表已启动或承诺交付日期。
...
| P2 | 桌宠勿扰、安静时段、提醒队列和好感度可靠性已修复并测试，见[专项记录](archive/audits/companion-knowledge-build-2026-09-10.md)；日程小剧场与演出数据化待深化 | 复用 affection、speech、Live2D 生命周期，增加受控事件绑定，并做已安装程序的语音/事件联动验收 | 勿扰/安静时段生效，空闲低功耗，动作许可与音画同步无回归 |
...
依据：[项目评价与优化／修复清单](archive/audits/project-review-checklist-2026-09-10.md)；[机器复现证据](evidence/project-review-2026-09-10.json)；办公机修复与门禁复核见[请求层修复记录](archive/audits/request-layer-repair-2026-09-10.md)。初始代码基线为 `1106904`，数据源合并 `576e0e1`。R1／R2／R3／O1／O2／O4 已在办公机修复并回归（`DATA_VERSION` 也一并同步），下表只保留仍未完成项；未完成项不因本轮文档交付而记为通过。

| ID／优先级 | 当前状态与事项 | 下一步 | 验收条件 |
| --- | --- | --- | --- |
| P0 | 定稿保护基线漂移 35/100：2026-09-10 的提示词批次改写了 pinned 场景的 `prompt`／`negative`／`animaCaption`，已提交源文件与 `data/prompt-pinned-scenes.json` 不再一致，`pin-scene-prompts.js --check` 失败；见[复核记录](archive/audits/request-layer-repair-2026-09-10.md) | 需用户决策：按基线回滚这些字段，或对已验证场景补真实出图后再 `scenes:pin-capture`；在此之前暂停改写 pinned 场景的批量任务 | `pin-scene-prompts.js --check` 通过；每条定稿场景改动都可追溯到真实出图证据 |
| P1 | 数据内容契约：unit 套件 15/444 失败（服装环境词泄漏、`kitagawa_marin` 缺安全默认蓝图等），`content-contracts` 2 项 pollution，均来自已提交数据 | 逐条定位来源批次并修正内容，重跑 unit 与 content-contracts，同步 `DATA_VERSION` | 15 项恢复通过、污染清零，两条门禁全绿 |
| V1／P2 | 待浏览器复验：Live2D 直达和 sessionStorage 不可用时的 CSP 切换 | 使用全新浏览器上下文检查直达、返回与存储异常 | 进出 Live2D 的文档策略正确，不扩大全站 unsafe-eval |
| O3／P2 | 性能优化：本轮实测绘图路由余量仍薄（PromptBuilderView JS 138.6/140.0 KiB、静态闭包 538.1/580.0 KiB，仅 4 项 >90% 警告），未放宽预算 | 优先按需加载低频动作，记录路由与静态闭包余量、冷／热进入与首个操作耗时 | 不放宽原硬预算；冷／热进入和首次操作有实测 |
| O5／P3 | 体验优化，非已证实 UI 缺陷：首次出图旅程与品牌文案 | 沿用 DESIGN，统一关键步骤的主行动、空态、等待及错误表达 | 深浅主题／窄屏／键盘可完成角色→场景→生成→入册 |
| O6／P3 | 维护优化：在已有本地日志上增加主动导出的脱敏诊断包 | 串起任务 ID、数据版本、请求、取消与结果 | 不默认外传，不泄漏 token、API key、聊天或图片 |

报告中 C1 对应上表原有故事／提示词任务：办公机的静态与门禁复核已暴露 pinned 场景被批次改写（上表 P0），需先决定回滚或补证，再继续逐条编译对账；人工看图继续后置，未看图不记为视觉通过。C2 对应角色／素材及批次对账：`576e0e1` 已合并的 16 蓝图／4 服装批次不再记为草稿，但数量与审核状态须重建后逐 ID 核对，不能仅凭合并说明认定全部验收。聚合数据是被忽略的生成物，不要求提交它们；`DATA_VERSION` 已按当前产物同步，后续改数据仍须重建并复核。

V2 沿用原 P0 的发布前默认素材验收，V3 沿用原 P1 的原生取消／语音与桌面验收，不另开重复任务，也不为本次文档交付启动 GPU 或安装程序。R1 与同模块回归已在办公机收口；建议下一步优先处理 pinned 基线决策与数据内容契约，素材／真机验收在既定环境进行。
