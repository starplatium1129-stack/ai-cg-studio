...
| --- | --- | --- | --- |
| P0 | 双主题门禁缺口：浅色已恢复，check-contrast.js 仍只读深色 | 扩展现有对比度门禁到浅色及角色强调色，沿用现有样式测试 | 两主题真实计算样式达到 AA，禁用字/浮层/图片遮罩视觉回归通过 |
...
| P0 | Node 脚本单测导入失败：本轮 test-compare-snapshots 无法解析 useFocusTrap 的 @/composables | 复用现有测试加载机制或统一可被 Node 解析的导入路径，先复现再改 | npm run test:unit 在当前提交通过，不能用前端 Vitest 通过替代 |
| P1 | 参考库尚有 245 条 pending；热门 586 形态与参考登记 397 形态口径不同 | 按角色 ID/outfit ID 对账，先登记遗漏，再复用 reference:render/reference:design 补齐，定向视觉审核 | 每个声明完成的形态具备 7 视角、可访问 URL 与审核证据，不把占位算完成 |
...
| P1 | 旧角色扩容与 DNA 重构规划混有已接入角色 | 按当前 116 个热门 ID 对候选去重；只为尚缺角色或已有角色明确缺陷立项 | 接入六层闭环齐全；不重做已完成扩容，不为数量套用模板 |
