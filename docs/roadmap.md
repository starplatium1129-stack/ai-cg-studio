...
| P1 | 参考库仍有 pending，且热门形态与参考登记形态口径不同；规模见项目状态 | 按角色 ID/outfit ID 对账，复用 reference:render/reference:design 补齐，包括第五批新增形态，定向视觉审核 | 每个声明完成的形态具备 7 视角、可访问 URL 与审核证据，不把占位算完成 |
...
| P1 | 热门角色按用户确认的新方案分批接入全年龄内容；第五批新增数据已写入，资产/发布与其余批次未完成 | 按[第五批接入进度](popular-sfw-batch5-progress-2026-09-08.md)先收口验收，再处理已有角色补差和第六至第九批 | 接入六层闭环齐全；不重做已有内容、不为数量套用模板，未审核资产不算完成 |
