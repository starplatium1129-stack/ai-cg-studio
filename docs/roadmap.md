...
| --- | --- | --- | --- |
| P1 | 内容门禁已按[本轮记录](archive/audits/content-gate-repair-2026-09-10.md)修复并通过结构模式完整回归；11 条新增服装覆盖场景与两处环境词迁移尚未真实出图 | 主力机按证据 ID 生成候选，人工看图继续后置；继续原 467 条语义审计，不将本轮门禁通过视为全场景完成 | 编译与真实模型参数匹配，人工决定和发布分别留痕，默认素材模式验收通过 |
...
| P2 | 办公机本地工具限制：① 浅克隆（`.git/shallow`）使 `pin-scene-prompts.js` 的 `--report`／`--apply`／`--capture` 全部报错，本次只能从基线文件回滚；② `test-control-failure-contract.js` 子用例 6／7 因 `managed-comfyui.ps1:92` 的 `taskkill.exe … \| Out-Null` 报 `CantActivateDocumentInPipeline` 而稳定失败（同一测试 21:05 尚可通过），见[复核记录](archive/audits/request-layer-repair-2026-09-10.md) | ① 需要逐条来源比对或重新固化基线时先 `git fetch --unshallow`，或让工具支持“无历史时以基线为目标”；② 在主力机复验后用不依赖管道的方式调用 taskkill（例如 `$null = & taskkill.exe … 2>&1`） | ① 三种模式在本机可运行或有明确降级提示；② 该契约文件在主力机与办公机均通过 |
...
V2 沿用原 P0 的发布前默认素材验收，V3 沿用原 P1 的原生取消／语音与桌面验收，不另开重复任务，也不为本次文档交付启动 GPU 或安装程序。R1 与同模块回归已在办公机收口，定稿保护的处置也已写入记录；内容门禁与补丁保护的工程回归已收口，下一步继续原场景审计与本轮候选出图；素材／真机验收在既定环境进行。
