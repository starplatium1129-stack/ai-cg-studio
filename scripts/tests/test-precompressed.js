...
async function fixture(run) {
  ...
  const write = (file, value = '{"version":1}', compressed = true) => {
  ...
  app.use('/data', (req, res, next) => { if (!require('../../server/public-data').includes(req.path.slice(1))) return res.sendStatus(404); next(); }, express.static(path.join(root, 'data')));
  const server = http.createServer(app);
  await new Promise(resolve => server.listen(0, '127.0.0.1', resolve));
  const get = (url, accept) => new Promise((resolve, reject) => {
