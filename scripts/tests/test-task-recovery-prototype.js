...
const { RecoveryJournal, RecoveryCoordinator, digest } = require('./prototypes/task-recovery');
...
function fixture(t) {
  ...
  const backend = {
    ...
    async cancelById(id, key) { calls.cancel.push({ id, key }); this.state = 'cancelled'; },
  ...
  return {
    ...
    advance(ms) { timestamp += ms; }, get now() { return timestamp; },
  };
}
