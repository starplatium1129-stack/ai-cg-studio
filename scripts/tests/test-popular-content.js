...
test('popular data: 47 characters, unique ids, exactly one default outfit per character', function () {
  assert.strictEqual(characters.length, 47, 'must ship exactly 47 characters');
  var ids = new Set(characters.map(function (character) { return character.id; }));
  assert.strictEqual(ids.size, 47, 'character ids must be unique');
...
test('blueprints: 47 characters x (6-7 prototype + 4-5 adult), all owned by a character, adult blueprints fail closed for non-adults', function () {
  ...
  assert.strictEqual(blueprints.length, 485, 'expected 485 character scenes, got ' + blueprints.length);
  ...
  assert.deepStrictEqual(sceneDist, { 10: 34, 11: 12, 13: 1 }, 'scene distribution must be 34x10 + 12x11 + 1x13');
  ...
  assert.deepStrictEqual(adultDist, { 4: 36, 5: 11 }, 'adult distribution must be 36x4 + 11x5');
