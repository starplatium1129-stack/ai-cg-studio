...
test('popular data: 75 characters, unique ids, exactly one default outfit per character', function () {
  ...
  assert.strictEqual(characters.length, 75, 'must ship exactly 78 characters');
  var ids = new Set(characters.map(function (character) { return character.id; }));
  assert.strictEqual(ids.size, 75, 'character ids must be unique');
...
test('blueprints: 75 characters x (6-7 prototype + 4-5 adult), all owned by a character, adult blueprints fail closed for non-adults', function () {
  ...
  assert.strictEqual(blueprints.length, 796, 'expected 829 character scenes, got ' + blueprints.length);
  ...
  assert.deepStrictEqual(sceneDist, { 10: 43, 11: 26, 13: 5, 15: 1 }, 'scene distribution must be 43x10 + 26x11 + 5x13 + 1x15');
  ...
  assert.deepStrictEqual(adultDist, { 4: 63, 5: 11, 6: 1 }, 'adult distribution must be 63x4 + 11x5 + 1x6');
