...
test('popular data: 44 characters, unique ids, exactly one default outfit per character', function () {
  assert.strictEqual(characters.length, 44, 'must ship exactly 44 characters');
  var ids = new Set(characters.map(function (character) { return character.id; }));
  assert.strictEqual(ids.size, 44, 'character ids must be unique');
...
test('blueprints: 44 characters x (6-7 prototype + 4-5 adult), all owned by a character, adult blueprints fail closed for non-adults', function () {
  ...
  assert.strictEqual(blueprints.length, 452, 'expected 452 character scenes, got ' + blueprints.length);
  ...
  assert.deepStrictEqual(sceneDist, { 10: 34, 11: 9, 13: 1 }, 'scene distribution must be 34x10 + 9x11 + 1x13');
  ...
  assert.deepStrictEqual(adultDist, { 4: 36, 5: 8 }, 'adult distribution must be 36x4 + 8x5');
