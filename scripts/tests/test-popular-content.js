...
test('popular data: 88 characters, unique ids, exactly one default outfit per character', function () {
  ...
  assert.strictEqual(characters.length, 88, 'must ship exactly 88 characters');
  var ids = new Set(characters.map(function (character) { return character.id; }));
  assert.strictEqual(ids.size, 88, 'character ids must be unique');
...
test('blueprints: 75 characters x (6-7 prototype + 4-5 adult), all owned by a character, adult blueprints fail closed for non-adults', function () {
  ...
  assert.strictEqual(blueprints.length, 939, 'expected 939 character scenes, got ' + blueprints.length);
  ...
  // 每个角色 10、11、13 或 15 个场景：10=6 原型+4 成人（43 角色）、11=7 原型+4 成人（39 角色）、
  ...
  assert.deepStrictEqual(sceneDist, { 10: 43, 11: 39, 13: 5, 15: 1 }, 'scene distribution must be 43x10 + 39x11 + 5x13 + 1x15');
  ...
  assert.deepStrictEqual(adultDist, { 4: 76, 5: 11, 6: 1 }, 'adult distribution must be 76x4 + 11x5 + 1x6');
