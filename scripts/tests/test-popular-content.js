...
test('popular data: 87 characters, unique ids, exactly one default outfit per character', function () {
  ...
  // 2026-09-02 天降与出包专栏：新增 5 位核心角色（伊卡洛斯/金色暗影/菈菈/茉茉/古手川唯），82 -> 87。
  assert.strictEqual(characters.length, 87, 'must ship exactly 87 characters');
  var ids = new Set(characters.map(function (character) { return character.id; }));
  assert.strictEqual(ids.size, 87, 'character ids must be unique');
...
test('blueprints: 75 characters x (6-7 prototype + 4-5 adult), all owned by a character, adult blueprints fail closed for non-adults', function () {
  ...
  // 2026-09-02 天降与出包专栏：新增 5 位核心角色（5 位角色各 11 蓝图 = +55 场景，87 角色 = 928 场景）。
  assert.strictEqual(blueprints.length, 928, 'expected 928 character scenes, got ' + blueprints.length);
  ...
  // 每个角色 10、11、13 或 15 个场景：10=6 原型+4 成人（43 角色）、11=7 原型+4 成人（38 角色）、
  ...
  assert.deepStrictEqual(sceneDist, { 10: 43, 11: 38, 13: 5, 15: 1 }, 'scene distribution must be 43x10 + 38x11 + 5x13 + 1x15');
  ...
  assert.deepStrictEqual(adultDist, { 4: 75, 5: 11, 6: 1 }, 'adult distribution must be 75x4 + 11x5 + 1x6');
