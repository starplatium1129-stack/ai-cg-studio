...
def build_cover(entries: list[dict], curated_ids: list[str], target: Path, version_label: str = "宁宁 / 夏目") -> None:
    ...
    draw.text((72, height - 108), "完整内容见 index.html；R18 默认模糊，悬停即可预览。", fill="#776e7b", font=load_font(24))
    target.parent.mkdir(parents=True, exist_ok=True)
...
.art{{position:relative;display:block;aspect-ratio:3/4;background:#e9e2df;overflow:hidden}} .art img{{width:100%;height:100%;object-fit:cover;object-position:50% 34%;transition:filter .3s,transform .3s}} .card.r18 .art img{{filter:blur(16px) saturate(.8);transform:scale(1.08)}} .card.r18 .art::after{{content:"R18 · 悬停预览";position:absolute;inset:50% auto auto 50%;transform:translate(-50%,-50%);padding:8px 12px;border-radius:999px;background:#30232ccc;color:#fff;font-size:12px;white-space:nowrap}} .card.r18:hover .art img{{filter:none;transform:scale(1.02)}} .card.r18:hover .art::after{{opacity:0}}
...
<script>const scenes={encoded};let character='all',rating='all';const grid=document.querySelector('#grid'),count=document.querySelector('#count'),q=document.querySelector('#q'),viewer=document.querySelector('#viewer'),full=document.querySelector('#full');
function esc(s){{return String(s).replace(/[&<>\"']/g,c=>({{'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}}[c]))}}
function render(){{const term=q.value.trim().toLowerCase();const list=scenes.filter(s=>(character==='all'||s.char===character)&&(rating==='all'||s.rating===rating)&&(!term||[s.id,s.title,s.story,s.category,s.characterLabel].join(' ').toLowerCase().includes(term)));count.textContent=`显示 ${{list.length}} / {total} 个场景 · R18 默认模糊`;grid.innerHTML=list.map(s=>`<article class="card${{s.rating==='R18'?' r18':''}}"><a class="art" href="${{s.image}}" data-full="${{s.image}}"><img loading="lazy" src="${{s.thumb}}" alt="${{esc(s.title)}}"></a><div class="body"><div class="meta"><span class="tag">${{esc(s.id)}}</span><span class="tag">${{esc(s.characterLabel)}}</span><span class="tag">${{esc(s.ratingLabel)}}</span><span class="tag">第 ${{s.attempt}} 次通过</span></div><h2>${{esc(s.title)}}</h2><p>${{esc(s.story)}}</p></div></article>`).join('')||'<div class="empty">没有找到匹配场景</div>';document.querySelectorAll('[data-full]').forEach(a=>a.onclick=e=>{{e.preventDefault();full.src=a.dataset.full;viewer.showModal()}})}}
q.oninput=render;document.querySelectorAll('[data-char]').forEach(b=>b.onclick=()=>{{document.querySelectorAll('[data-char]').forEach(x=>x.classList.remove('active'));b.classList.add('active');character=b.dataset.char;render()}});document.querySelectorAll('[data-rating]').forEach(b=>b.onclick=()=>{{document.querySelectorAll('[data-rating]').forEach(x=>x.classList.remove('active'));b.classList.add('active');rating=b.dataset.rating;render()}});document.querySelector('#closeViewer').onclick=()=>viewer.close();render();</script></body></html>"""
