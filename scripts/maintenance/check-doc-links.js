...
function check() {
  ...
  const redirects = JSON.parse(fs.readFileSync(path.join(root, 'docs/redirects.json'), 'utf8'));
  for (const [old, target] of Object.entries(redirects)) {
    if (!old.startsWith('/docs/') || !target.startsWith('/docs/') || target.includes('..') || !fs.existsSync(path.join(root, target.slice(1)))) errors.push('Invalid document redirect: ' + old + ' -> ' + target);
  }
