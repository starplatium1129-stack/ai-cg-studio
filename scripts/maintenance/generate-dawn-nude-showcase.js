...
/**
 * generate-dawn-nude-showcase.js
 * 为新成人场景「圣园未花 · 晨曦寝所纯白裸态」(misono_mika_dawn_nude_bedroom)
...
async function main() {
  console.log(`[Dawn Nude Showcase] 开始生成圣园未花全新专属成人场景样张...`);

  const character = popular.findCharacter(popularChars, 'misono_mika');
  const bp = blueprintsData.find(b => b.id === 'misono_mika_dawn_nude_bedroom');
  if (!character || !bp) throw new Error('未找到未花或晨曦寝所纯白裸态场景蓝图');
  ...
  console.log(`[Dawn Nude Showcase] 提示词: ${prompt.slice(0, 200)}...`);
  ...
  const seed = 1065372979; // 02994 黄金种子

  console.log(`[Dawn Nude Showcase] 提交标准渲染任务 (seed: ${seed})...`);
  ...
  console.log(`[Dawn Nude Showcase] Job ID: ${jobId}, 等待生成完成...`);
  const imgBuf = await pollJob(jobId);
  fs.writeFileSync(tempPng, imgBuf);
  console.log(`[Dawn Nude Showcase] 基础图生成完成 (${imgBuf.length} 字节)`);
  ...
  console.log(`[Dawn Nude Showcase] 展示库大盘转换完成: images & thumbs`);
  ...
  const newEntry = {
    ...
    provenance: {
      ...
      review: {
        verdict: 'pass',
        recordId: `popular:${character.id}:${bp.id}@attempt-1`,
        notes: '圣园未花全新晨曦寝所纯白裸态NSFW场景样张入库',
  ...
  console.log(`[Dawn Nude Showcase] Manifest 登记成功！当前条目总数: ${manifest.entries.length}`);

  // 3. 同时渲染一张 Remacri 2x Hires 放到用户桌面！
  console.log(`[Dawn Nude Showcase] 正在为该新 NSFW 场景生成 Remacri 2x Hires 超分版放到桌面...`);
  ...
  console.log(`[Dawn Nude Showcase] Hires Job ID: ${hiresJobId}, 等待超分完成...`);
  const hiresBuf = await pollJob(hiresJobId);
  const desktopOut = path.join(DESKTOP_DIR, 'mika_dawn_nude_bedroom_hires.png');
  fs.writeFileSync(desktopOut, hiresBuf);
  console.log(`[Dawn Nude Showcase] 超清 Hires 原图已保存至桌面: ${desktopOut} (${hiresBuf.length} 字节)`);
