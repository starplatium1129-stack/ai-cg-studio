...
function resolveSdkRoot(root = ROOT, env = process.env, exists = fs.existsSync) {
...
function desktopBuildEnvironment(root = ROOT, source = process.env) {
  const env = { ...source };
...
function inspectDesktopBuildEnvironment(options = {}) {
  const root = options.root || ROOT;
  const env = desktopBuildEnvironment(root, options.env || process.env);
  const checks = [];
  const add = (name, ready, detail, help) => checks.push({ name, ready: Boolean(ready), detail, ...(help ? { help } : {}) });
...
module.exports = { resolveSdkRoot, desktopBuildEnvironment, inspectDesktopBuildEnvironment, assertDesktopBuildEnvironment };
