"""Run the fixed-seed blind gate for the isolated Nene v17 core LoRA.

This reuses the image generation and sheet compositor from the v16 audit, but
uses corrected school-uniform prompts (a visible yellow bow is not treated as
an error) and makes ordinary, non-R18 rendering an explicit hard gate.

Before running, copy the selected v17 checkpoint into the WebUI LoRA directory
as ``ayachi_nene_v17_core_eval.safetensors``.  This script intentionally never
loads the R18 add-on: the primary question is whether core alone preserves
identity and canonical clothing without adult-context leakage.
"""
...
SPEC = importlib.util.spec_from_file_location("nene_v17_base_audit", SOURCE_PATH)
...
OUTPUT_ROOT = Path(
    r"E:\code\2\lora\AI\Reviews\ModelEvaluations\nene_v17_core_gate_2026-07-29"
)
OUTPUT_TAG = os.environ.get("NENE_V17_GATE_TAG", "final")
if not re.fullmatch(r"[a-z0-9][a-z0-9_-]{0,63}", OUTPUT_TAG):
    raise RuntimeError("NENE_V17_GATE_TAG must use lowercase letters, numbers, _ or -")

EVAL_LORA_PATH = Path(
    r"E:\code\2\lora\AI\Data\Models\Lora\ayachi_nene_v17_core_eval.safetensors"
)
...
SOURCE.CANDIDATES = [
    ...
    SOURCE.Candidate("v17-0.65", "ayachi_nene_v17_core_eval", 0.65),
    SOURCE.Candidate("v17-0.75", "ayachi_nene_v17_core_eval", 0.75),
]
...
SOURCE.TESTS = [
    ...
    SOURCE.Test(
        "witch-exact-legwear",
        f"{quality}, {identity}, full_body, standing, official_witch_outfit, "
        "witch_hat, pink_hat_band, large_pink_hat_bow, black_cape, pink_lining, "
        "pink_crossover_top, black_skirt, asymmetric_legwear, "
        "black_white_striped_thighhigh, exactly_one_black_and_white_striped_thighhigh, "
        "bare_other_leg, "
        "white_frilled_anklet, black_strappy_boots, holding_silver_handgun_downward, "
    ...
    SOURCE.Test(
        ...
        f"{quality}, {identity}, full_body, dynamic_three_quarter_pose, official_witch_outfit, "
        "witch_hat, pink_hat_band, black_cape, pink_lining, pink_crossover_top, black_skirt, "
        "asymmetric_legwear, black_white_striped_thighhigh, "
        "exactly_one_black_and_white_striped_thighhigh, bare_other_leg, "
        "white_frilled_anklet, black_strappy_boots, holding_silver_handgun_downward, "
        "casting_purple_magic, moonlit_classroom, cinematic_rim_light, detailed_background",
        refs / "nene_stand_01.png",
        "full",
        "two_striped_stockings, symmetric_legwear, black_solid_thighhigh, long_white_stocking, "
...
def blind_mapping(test_name: str, seed: int) -> dict[str, str]:
    """Use a v17-specific seed so published v16 mappings cannot unblind it."""

    versions = [candidate.code_name for candidate in SOURCE.CANDIDATES]
    random.Random(f"nene-v17-core-gate-{test_name}-{seed}-2026-07-29").shuffle(
        versions
    )
...
def describe_evaluated_lora() -> dict[str, str]:
    """Bind every gate output to the exact local artifact that produced it."""

    if not EVAL_LORA_PATH.is_file():
        raise RuntimeError(
            "missing evaluated LoRA artifact: "
            f"{EVAL_LORA_PATH}; copy the selected checkpoint before running"
        )
...
def main() -> None:
    ...
    if artifact_record.exists():
        ...
        if prior != artifact:
            raise RuntimeError(
                ...
                "choose a new NENE_V17_GATE_TAG"
            )
    else:
    ...
    manifest = {
        "schema": "ai-cg-studio.nene-v17-core-manual-gate.v1",
        "purpose": "fixed-seed blinded comparison of v15, v16 and isolated v17 core",
        ...
        "priority": {
            ...
            "witch_outfit_and_asymmetric_legwear": 0.25,
