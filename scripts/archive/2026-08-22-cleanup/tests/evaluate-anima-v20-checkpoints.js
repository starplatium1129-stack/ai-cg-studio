...
/* Real-GPU production-workflow audit for the scientific Anima v20 shortlist. */
...
var OUTPUT_ROOT = process.env.AICS_ANIMA_V20_AUDIT_DIR || path.join(
  AI_ROOT,
  'Reviews',
  'AnimaV20CheckpointAudit',
  '2026-08-09_baseline-a_e08_e12_vs_v19'
);
var LORA_ROOT = path.join(AI_ROOT, 'ComfyUI', 'models', 'loras');
var SCENE_IDS = ['sc260', 'sc261', 'sc262', 'sc263', 'sc264', 'sc265'];
var CANDIDATES = [
  { id:'e08', epoch:8, step:336, file:'ayachi_nene_v20_anima_scientific_a_e08_audit.safetensors' },
  { id:'e12', epoch:12, step:504, file:'ayachi_nene_v20_anima_scientific_a_e12_audit.safetensors' },
];
var CLIENT_ID = 'aics-anima-v20-checkpoint-audit-' + crypto.randomUUID();
...
function workflowFor(record, candidate) {
  ...
  workflow['10'].inputs.filename_prefix = [
    'anima_v20_checkpoint_audit', candidate.id, record.sceneId, 'seed-' + record.seed,
  ].join('/');
...
async function main() {
  ...
  var manifest = fs.existsSync(manifestFile) ? readJson(manifestFile) : {
    version:1,
    purpose:'Compare scientific Anima v20 baseline-A epochs 8/12 against the current production v19 checkpoint',
    comfy:COMFY,
  ...
  writeJson(manifestFile, manifest);
  console.log('Anima v20 checkpoint audit manifest: ' + manifestFile);
}
