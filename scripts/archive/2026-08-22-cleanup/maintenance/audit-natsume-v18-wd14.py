"""Run a fixed-seed blind identity gate for Natsume v18 versus v15 and v17."""
...
SPEC = importlib.util.spec_from_file_location("natsume_v18_base_audit", SOURCE_PATH)
...
OUTPUT_ROOT = Path(r"E:\code\2\lora\AI\Reviews\ModelEvaluations\natsume_v18_wd14_gate_2026-07-30")
OUTPUT_TAG = os.environ.get("NATSUME_V18_GATE_TAG", "final")
if not re.fullmatch(r"[a-z0-9][a-z0-9_-]{0,63}", OUTPUT_TAG):
    raise RuntimeError("invalid NATSUME_V18_GATE_TAG")
EVAL_LORA_PATH = Path(r"E:\code\2\lora\AI\Data\Models\Lora\shiki_natsume_v18_wd14_eval.safetensors")
...
SOURCE.CANDIDATES = [
    ...
    SOURCE.Candidate("v18-0.65", "shiki_natsume_v18_wd14_eval", 0.65),
    SOURCE.Candidate("v18-0.75", "shiki_natsume_v18_wd14_eval", 0.75),
]
...
def blind_mapping(test_name: str, seed: int) -> dict[str, str]:
    ...
    random.Random(f"natsume-v18-wd14-{test_name}-{seed}-2026-07-30").shuffle(versions)
...
def main() -> None:
    ...
    manifest = {
        "schema": "ai-cg-studio.natsume-v18-wd14-manual-gate.v1",
        "purpose": "fixed-seed blinded v15/v17/v18 identity, body, qipao, safe and R18 comparison",
        ...
        "hard_gates": {"critical_identity_error": 0, "v18_face_not_worse_than_best_prior": True, "ordinary_prompt_r18_leakage": 0, "qipao_full_body_pass_rate": 0.67, "r18_full_body_materially_better_than_v17": True},
