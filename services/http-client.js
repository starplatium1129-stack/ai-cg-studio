...
function matchesNoProxy(host, noProxy) {
    ...
    for (const part of parts) {
        ...
        if (portIdx > 0 && !p.includes(']'))
            p = p.slice(0, portIdx);
        if (!p)
            continue;
        if (hostLower === p || hostLower.endsWith('.' + p))
            return true;
    }
    return false;
}
...
function resolveProxy(target) {
    if (matchesNoProxy(target.hostname, process.env.NO_PROXY || process.env.no_proxy))
        return null;
...
function request(baseUrl, pathname, options) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        function onAbort() {
            ...
            else if (connectReq && !connectReq.destroyed)
                connectReq.destroy(abortError());
        }
        function cleanupAbort() {
            if (opts.signal)
                opts.signal.removeEventListener('abort', onAbort);
        }
        function attachResponse(clientReq) {
            req = clientReq;
