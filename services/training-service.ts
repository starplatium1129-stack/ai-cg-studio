...
/** 每任务日志写入器：热路径只做内存追加，落盘合并为异步批量写（同步 I/O 不再阻塞事件循环）。 */
interface LogSink {
...
function createTrainingService(options: TrainingServiceOptions) {
  ...
  const logSinks = new Map<TrainingJobId, LogSink>();
   ...
   */
  function createLogSink(id: TrainingJobId): LogSink {
    ...
    async function maybeTruncate(): Promise<void> {
      let stat: fs.Stats | null = null;
      try {
      ...
      if (!stat || stat.size <= logMaxBytes) return;
      ...
      let tail: Buffer;
      try {
        const { buffer: readBuffer, bytesRead } = await handle.read(
          Buffer.alloc(length), 0, length, start
        );
    ...
    return {
      ...
      dispose: function (): void {
        ...
        if (immediate) { clearImmediate(immediate); immediate = null; }
        scheduled = false;
      },
    };
  }

  function logSinkFor(id: TrainingJobId): LogSink {
  ...
  async function getLogs(value: unknown, cursorValue: unknown, versionValue: unknown): Promise<{
    id: TrainingJobId;
    cursor: number;
    nextCursor: number;
    reset: boolean;
  ...
  function close(): void {
    ...
    for (const [id, child] of children) {
      ...
      if (state && state.pid) {
        try { killProcess(state.pid, child); } catch { /* 进程可能已退出 */ }
      }
    }
