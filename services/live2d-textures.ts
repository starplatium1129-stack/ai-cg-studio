...
const characters = new Set(['nene', 'natsume'])

/** Derived atlases are optional, bounded, and never overwrite imported model assets. */
export function createLive2dTextureService(rootDir: string) {
  ...
  function source(character: string, reference: string) {
    if (!characters.has(character) || !reference || path.isAbsolute(reference)) throw new Error('Invalid Live2D asset')
  ...
  function manifest(character: string, quality: string) {
    ...
    const model = JSON.parse(fs.readFileSync(source(character, character + '.model3.json'), 'utf8'))
    const refs = model.FileReferences
    if (!refs || !Array.isArray(refs.Textures)) throw new Error('Invalid Live2D manifest')
    const assetUrl = (reference: string) => {
      source(character, reference)
      return '/assets/live2d-current/' + character + '/' + reference.replace(/\\/g, '/').split('/').map(encodeURIComponent).join('/')
    }
    for (const key of ['Moc', 'Physics', 'Pose', 'DisplayInfo']) if (refs[key]) refs[key] = assetUrl(refs[key])
    for (const expression of refs.Expressions || []) if (expression.File) expression.File = assetUrl(expression.File)
    for (const motions of Object.values(refs.Motions || {}) as Array<Array<{ File?: string; Sound?: string }>>) {
      for (const motion of motions) {
        if (motion.File) motion.File = assetUrl(motion.File)
        if (motion.Sound) motion.Sound = assetUrl(motion.Sound)
      }
    }
    refs.Textures = refs.Textures.map((file: string, index: number) => {
  ...
  async function texture(character: string, quality: string, index: number) {
    ...
    const model = JSON.parse(fs.readFileSync(source(character, character + '.model3.json'), 'utf8'))
    const file = source(character, model.FileReferences?.Textures?.[index])
