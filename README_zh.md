...
系统不仅完整支持专属主角 **绫地宁宁** 与 **四季夏目**，还内置了覆盖明日方舟、原神、崩铁、芙莉莲、Fate、Re:Zero、俄语妹、青猪、刀剑神域、魔禁/超炮、约战等作品的 **116 位热门角色**（579 套服装形态，参考档案库 118 角色×397 已登记形态/2,779 参考条目，数字以 `DATA_VERSION` 时刻数据为准）。项目会把故事、角色、情绪、镜头、构图、光照和出图参数放在同一个 Scene 中，减少从空白 Prompt 开始反复试错的时间。
...
- **丰富的场景库与热门角色体系**：
  - 浏览和搜索 302 个场景（+1,247 蓝图）与 335+ 逐场景审核样张，按角色、分类、季节与内容分级筛选（全年龄 / R15 / R18）。
  - 116 位热门动漫/游戏角色，支持专属服装形态与私密全裸纯粹形态切换。
  - 在“效果样张”中查看逐场景审核的实际成图，支持带着场景直接进入导演台。
- **角色 4 视角参考档案库（Character Reference Bible）**：
  - 118 角色 $\times$ 397 已登记服装形态（共 2,779 个参考条目：2,534 条已发布 + 245 条待渲染占位）：四视角电影级标准参考——面部特写（85mm f/1.4）、3/4 半身定妆、正面动态全身立姿与 45° 侧后背影/回眸，外加 1,191 张三视图设计图基线（`reference:design` 渲染管线）。
