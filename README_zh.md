...
- 浏览和搜索 259 个场景，并按角色、分类、季节与内容分级筛选（全年龄 / R15 / R18）
- 在“效果样张”中查看 259 张逐场景审核的实际成图，按精选、角色和分级筛选，并直接带着场景进入导演台
...
- 使用结束：点击 **停止全部服务**。它会停止本地网关、分享隧道、GPT-SoVITS，以及由控制面板启动的 reForge；手动启动的 WebUI 不会被误关。
...
- `data/scenes.json`：由分片自动生成、供网页读取的 259 个场景
...
场景日常维护请编辑 `data/scenes/*.json`，不要直接修改自动生成的 `data/scenes.json`。完整流程见 [场景库维护手册](docs/maintenance.md)。
