...
- 默认提取日文台词或朗读日文故事，可切换中文演绎，并支持系统试听与本机 GPT-SoVITS 专用角色声线
...
双击 `control.bat`，确认 WebUI 地址后点击 **启动并生成分享链接**。当前电脑已配置并评测宁宁、夏目的本地 GPT-SoVITS 专用权重；网关会按角色排队切换模型，避免多人使用时串音。没有语音服务时，导演台的日文或中文系统声音试听仍可使用。
