...
- 在“角色对话”中连接本机 Ollama，与宁宁或夏目进行流式文字聊天；可复用现有 GPT-SoVITS 自动配音，并以轻量动态立绘反馈呼吸、眨眼和说话状态
...
日常增删场景、修改故事或替换样张，直接使用网站中的“更多 → 场景管理”。页面会暂存修改，点击“保存到项目”后自动备份、写入正确分片并运行检查，不要求维护者理解代码或手写 JSON。
