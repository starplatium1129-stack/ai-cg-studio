...
- **契约类型** — `types/*` + 渐进运行时：`services/control-operation.ts`、`services/serial-queue.ts`
- **E2E** — Playwright：`tests/e2e/`，`npm run test:e2e`
- **页面架构门禁** — `scripts/tests/test-page-architecture.js`（外置控制器、禁 inline handler、CSP 就绪）

### 仍待推进

- **TypeScript 渐进迁移** — 优先高风险服务与核心流水线；`tools/` 页面控制器仍以 JS/MJS 为主，勿整库重写
- **测试加深** — 在现有脚本测试上补 scene CRUD / voice pipeline / gallery 关键路径；CI 前保持本地 `validate` + 关键 e2e 绿
- **CI** — 尚无 `.github/workflows/`；目标：push 自动 lint + typecheck + unit/script tests（e2e 可选/夜间）
- **UI 一致性** — 热页统一 `nav-back` / `page-kicker` / `empty-state` 与 design-system atelier chrome
