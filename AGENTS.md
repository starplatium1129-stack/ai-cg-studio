...
- **TypeScript 渐进迁移** — 运行时服务已迁齐；`tools/` 页面控制器仍以 JS/MJS 为主，勿整库重写
- **测试加深** — 在现有脚本测试上补 scene 真实 CRUD 持久化 / live voice 管道；保持本地 `validate` + 关键 e2e 绿
- **CI 硬化** — 缓存与失败产物已加；e2e 可拆夜间若 push 过慢
- **UI 一致性** — 热页与 showcase/scenario/color-script/lora 已对齐 kicker；其余 docs 页可按需补 empty-state
