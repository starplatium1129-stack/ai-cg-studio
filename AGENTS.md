...
- **下载/安装必须先测速选源再开始（2026-08-16 用户教训）**：任何大文件下载（模型权重、pip 包、wheel）动手前，先用 curl 限时 8–10s 实测候选源直链速度（`curl -sL --max-time 10 -o NUL -w '%{http_code} %{size_download}' <url>`），选最快源再下载；**禁止直接开跑然后干等**。此坑两个并行会话均踩过（本会话：pytorch 官方源 0.2MB/s 白等 15 分钟；另一协作者：同样先开跑后等待，均在用户提醒后才改正）——**写进 AGENTS.md 正是因为这类错误会跨会话复发、会话记忆会丢**。本机实测速度参考（单流）：ModelScope ~32MB/s > 阿里云 pytorch-wheels 2.7MB/s（8 段并行 Range 可达 ~22MB/s）> pytorch 官方 CDN ~0.2MB/s；hf-mirror 会波动（当日实测 0.1–55MB/s 之间，用前必测）。大文件（>500MB）用 8 段并行分段下载 + 拼接 + 大小校验（模板：`scripts/maintenance/download-torch-cu130.ps1`，1.78GB 并行 79s 完成）。注意：部分镜像（如交大 pytorch-wheels）只是代理索引，页面链接仍回源官方 CDN，测速要以实际回源地址为准。

- **大模型 GGUF/权重标准下载流程（2026-08-21 起，免代理、一步到位）**：LLM 权重优先走 **ModelScope 直链**（本机实测单流 28~33MB/s，无需登录/代理；hf-mirror 波动大，用前必测）。完整流程：①列文件并取精确大小与 sha256——HF 用 `curl -s "https://huggingface.co/api/models/<repo>/tree/main?recursive=true"` 读 `lfs.size` 与 `lfs.oid`（**oid 即 sha256**）；ModelScope 同名仓库 `.../models/<ns>/<repo>/resolve/master/<file>` 先 `curl -s -o NUL -I <url>` 验 200 再定。②先测源再下载（`curl -sL --max-time 10 -o NUL -w '%{http_code} %{size_download}' <url>`），禁止直接开跑干等。③**拿真实总长别用 HEAD**：ModelScope resolve 会 302 到 `cdn-lfs-cn-1`，总长用 `curl -sL --range 0-0 -D - -o NUL <resolve-url>` 解析 `Content-Range: bytes 0-0/<total>`。④下载=每段一条任务**直接 `curl`**（`curl -sL --max-time 1800 -r <start>-<end> -o <part> <url>`），大文件 8 段并发；**禁止加 `--retry-all-errors`**（连接被并发压掉时会空转卡 0 字节），**禁止在 DSH 后台任务里套 `Start-Job` 调 curl**（进程管道被沙箱卡死，前台/直连才正常）；并发大段有几十秒慢启动，**不要 20~30s 抽查到 0 字节就误判卡死并 kill**，等完成通知即可。⑤收尾：逐段校长度 → 二进制拼接 → sha256 对 oid → 移入目标目录。现成工具：`E:\tools\llama-cpp\dl-seg.ps1`（单段下载）+ `E:\tools\llama-cpp\concat-verify.ps1`（拼接校验入库）；完整坑记录看 `docs/download-mirror-lessons.md`（2026-08-21：Qwen3.8-27B 两个 GGUF 均按此流程完成并 sha256 验过）。
...
- 角色配音是现有功能，重构布局时不得删除。
- 网站 `/chat` 的 Live2D 默认按需加载，用户显式启用前不得下载大贴图；桌面 Companion（Tauri 壳）是明确例外：窗口可见启动时默认加载，`--hidden` 与用户显式关闭时不得下载。
...
- 桌面打包模式限制：Tauri 壳仅在打包模式注入 `AICS_DESKTOP_PACKAGED=1`（`main_shared.rs` gateway_env；dev 模式不注入）。该模式下 data 位于只读应用包、维护脚本未打包、系统 npm/node 读不了包内文件，故场景内容维护链路（`/api/maintenance/scenes`、`/api/maintenance/run`、`/api/maintenance/build-web`）返回 501 `DESKTOP_MAINTENANCE_UNAVAILABLE`；`/api/maintenance/showcase` 与 `home-hero` 写 AI 工作区不受限。前端 SceneManagerView/ControlView 通过 `companionDesktop.isPackaged()` 禁用入口。测试锚点：test-gateway-contract.js「desktop mode 501」。
...
- **UI 视觉打磨（2026-08-21 全部完成）**：首页 05/ARCHIVE 孤卡改横幅入口 + 区块间距收紧、角色页作品 pill 单角色项折叠、控制台公网 CTA 去 btn-block 收敛、聊天页次要操作收进「更多」菜单（`--z-popover` 层级 + 毛玻璃，外点/Esc 关闭）；Showcase 过滤器两行 pill 降噪（类型/角色筛选收进下拉并与搜索/分级对齐）；Director（/prompt-builder）顶部「角色高质量路线」callout 支持默认折叠与一键关闭并持久化状态（`aics_managed_route_*`）。
- **角色局部换装（Anima Inpaint / Outfit Swap）全链路验证通过（2026-08-21）**：手绘精确遮罩与 CLIPSeg 文本自动识别（灵敏度阈值 slider 0.20-0.80 可调，默认 0.45）+ 预设衣橱 + 角色专属 LoRA/通用模式 + ComfyUI `ImageCompositeMasked` 像素级保真回贴 + 2x 超分放大打通；契约测试 `test-anima-routes.js` 与 E2E `anima-quick.spec.ts` 均 100% 绿灯。
