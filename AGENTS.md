...
- **UI 视觉打磨（2026-08-21 已完成四项，前后对比截图 `.review-shots/before-*.png` vs 现行）**：首页 05/ARCHIVE 孤卡改横幅入口 + 区块间距收紧、角色页作品 pill 单角色项折叠、控制台公网 CTA 去 btn-block 收敛、聊天页次要操作收进「更多」菜单（`--z-popover` 层级 + 毛玻璃，外点/Esc 关闭）。**剩余两项待做**：① Showcase 过滤器两行 pill 降噪——类型/角色筛选收进下拉或抽屉，外露只留搜索+分级；② Director（/prompt-builder）顶部「角色高质量路线」callout 默认折叠为可关闭横幅并持久化关闭状态。
