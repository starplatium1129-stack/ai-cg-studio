...
证据与版本差异见 [调研记录](../../../docs/research/prompts/prompt-skill-research-2026-09-09.md)。仅在需要查模型能力、解释参数来源或处理旧规范冲突时读取；项目字段约束另见 `docs/engineering-contracts.md`。不要把历史指南当作高于当前 skill 与当前实现的执行规范。
...
- 可以并行且边界独立时再拆分子任务；每个子任务只提供完成它所需的最小上下文，主任务统一核对身份、服装、编译结果与交付证据。
...
交付说明列出改动条目、依据、当次执行的检查、出图证据与未完成项。
