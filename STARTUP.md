...
- **本机试听**：调用浏览器的系统中文声音，用来检查文本和语速，不需要额外安装。
- **AI 声线生成**：由本机 GPT-SoVITS 生成 WAV；朋友通过分享链接使用时，计算仍在你的电脑上完成。

当前电脑已在相邻的 `AI/Voice/` 目录配置好宁宁和夏目的参考音频。双击 `control.bat` 时会自动检查并启动语音服务；也可单独运行：
...
参考音频的语言应设为日语 `ja`，目标台词设为中文 `zh`。保存后，导演台会自动检测服务并提取场景中的 `「角色台词」`；也可切换为朗读完整故事。请只使用你有权使用的声音模型和参考音频。
