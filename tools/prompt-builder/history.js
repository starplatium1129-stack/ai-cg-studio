...
function migrateHistory(list){
  ...
  var out=list.filter(function(h){return h&&typeof h==='object';}).map(function(h){
    ...
    if(typeof e.image_data!=='string'){e.image_data='';changed=true;}
...
async function saveHistoryWithRating(rating,favorite,notes,imageUrl,imageId,useActual,imageData){const entry=baseEntry(!!useActual);entry.rating=rating;entry.favorite=!!favorite;entry.notes=notes||'';entry.image_id=imageId||'';entry.image_url=normalizeImageUrl(imageUrl);entry.image_data=typeof imageData==='string'&&imageData.startsWith('data:image/')?imageData:'';await commitHistoryEntry(entry);}
...
function renderHistory(){
  ...
  list.innerHTML=visibleHistory.map(function(h){
    ...
    return '<article class="history-item"><div class="history-thumb"><img class="'+(h.image_url?'':'history-placeholder')+'" src="'+escapeHtml(src)+'" data-fallback="'+fallback+'"'+imageAttrs+' alt="'+title+' 缩略图" loading="lazy" data-action="history-fallback"><span class="history-thumb-badge">'+(hasImage?'生成图':'Prompt 快照')+'</span></div><div class="history-main"><div class="history-card-title">'+title+'</div><div class="history-text" title="'+escapeHtml(h.prompt||'')+'">'+escapeHtml(h.story||h.prompt||'')+'</div><div class="history-meta"><span class="primary">'+character+'</span><span class="sep" aria-hidden="true">·</span><span>v'+version+'</span><span class="sep" aria-hidden="true">·</span><span>seed '+seed+'</span></div><div class="history-side"><div class="history-rating'+(avg?' rated':'')+'" aria-label="'+(avg?'平均评分 '+avg+' 分':'未评分')+'"><span class="star" aria-hidden="true">★</span><span>'+ratingText+'</span>'+(h.favorite?'<span class="favorite" title="已收藏" aria-label="已收藏">♥</span>':'')+'</div><div class="history-actions"><button type="button" class="history-action primary" data-action="continue-history" data-id="'+id+'">继续编辑</button><button type="button" class="history-action" data-action="copy-history" data-id="'+id+'" aria-label="复制提示词">复制</button><button type="button" class="history-action delete" data-action="delete-history" data-id="'+id+'" title="删除记录" aria-label="删除记录">×</button></div></div></div></article>';
  }).join('')+(state.history.length>visibleHistory.length?'<div class="history-empty"><button type="button" class="history-action" data-action="show-more-history">显示更早的 '+Math.min(HISTORY_PAGE_SIZE,state.history.length-visibleHistory.length)+' 条</button><div>全部 '+state.history.length+' 条记录均保存在本地</div></div>':'');
