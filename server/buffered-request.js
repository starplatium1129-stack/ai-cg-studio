...
function requestBuffered(target, options) {
  return new Promise((resolve, reject) => {
    ...
    const makeError = options.makeError || ((kind, error) => error || new Error(kind));
    ...
    const request = (url.protocol === 'https:' ? https : http).request(url, {
      ...
      response.on('end', () => {
        ...
        finish(null, { status: response.statusCode || 0, headers: response.headers, body: Buffer.concat(chunks, size) });
      });
