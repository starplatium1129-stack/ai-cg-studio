...
function validate(body) {
  ...
  loras = loras.map(function (item) {
    ...
    return { id:item.id, strength:number(item.strength, 'loraStrength', spec.min, spec.max, false), file:spec.file };
  });
...
function buildWorkflow(input) {
  ...
  graph[decoded] = { class_type:'VAEDecode', inputs:{ samples:[sample, 0], vae:[vae, 2] } };
...
async function probeWebUI(config) {
  try {
    ...
    return {
      online:true,
      checkpoint:options && options.sd_model_checkpoint || '',
      ...
      models:Array.isArray(models) ? models.map(function (item) { return item && (item.title || item.model_name || item.name); }).filter(Boolean) : []
    };
  } catch (e) {
    return { online:false, checkpoint:'', samplers:[], schedulers:[], models:[] };
  }
}
...
function createWebUIJob(config, input, ownerId) {
  ...
  void requestJson(config, 'SD_HOST', 'POST', '/sdapi/v1/txt2img', payload, 20 * 60 * 1000).then(function (result) {
    ...
    webJob.metadata.seed = result.info && result.info.seed || input.seed;
...
function createGenerationRouter(config, dependencies) {
  ...
  var comfy = dependencies.waiComfy || anima.createAnimaService(config, { buildWorkflow:buildWorkflow, outputPrefix:OUTPUT_PREFIX, outputNodeId:'10', mediaNamespace:'wai', engine:'sd', routeBase:'/api/generation' });
  var jobs = new Map();
  var router = express.Router();
  var limit = security.rateLimit({ capacity:12, refillMs:5000, label:'WAI 出图' });

  function status() {
    return { online:false, provider:'webui', webuiOnline:false, comfyFallbackOnline:false, checkpoint:'', samplers:[], schedulers:[], models:[], loras:Object.keys(LORAS).map(function (id) { return { id:id, file:LORAS[id].file, character:LORAS[id].character }; }), capabilities:{ basic:true, hires:false, faceDetailer:false }, pending:comfy.status().pending, maxPending:MAX_PENDING };
  }
  router.get('/api/generation/status', async function (req, res) {
    ...
    data.checkpoint = webui.checkpoint || (comfyOnline ? CHECKPOINT : '');
  ...
  router.post('/api/generation/jobs', limit, express.json({ limit:MAX_BODY }), async function (req, res) {
    ...
    if (webui.online && (!webuiSupportsSampler || !webuiSupportsScheduler)) {
    ...
    if (input.faceDetailer || input.hiresFix) {
      return envelope.fail(res, 503, '当前功能需要在线的 SD WebUI / reForge', { code:'WEBUI_REQUIRED_OFFLINE' });
    }
    var provider = 'comfy';
    if (!comfyOnline) return envelope.fail(res, 503, 'SD WebUI 与 ComfyUI 均不可用', { code:'GENERATION_BACKENDS_OFFLINE' });
    if (input.comfyUnsupported) return envelope.fail(res, 503, '当前请求不符合 ComfyUI basic fallback 能力', { code:'COMFY_CAPABILITY_UNAVAILABLE' });
    if (provider === 'comfy') {
...
module.exports = { createGenerationRouter:createGenerationRouter, validateInput:validate, buildWorkflow:buildWorkflow, constants:{ CHECKPOINT:CHECKPOINT, LORAS:LORAS, SAMPLERS:SAMPLERS } };
